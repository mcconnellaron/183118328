...
struct Ctxt<'a> {
    ...
    in_fun: bool,
    //add tail_position that keeps track
    tail_position: bool,
    //add loop tail stuff
    loop_tail_position: bool,
    //add value to track number of parameters (basically how far below the return pointer we go)
    num_params: u32,
}
...
impl<'a> Ctxt<'a> {
    ...
    fn with_params(params: &[Symbol]) -> Ctxt<'a> {
        let env = params
            .iter()
            .enumerate()
            .map(|(i, param)| (*param, mref![Rbp + %(8 * (i + 3))]))
            .collect();
        Ctxt {
            si: 0,
            curr_lbl: None,
            env,
            in_fun: true,
            //default is for function body to be tail position
            //the body needs to propigate this value during compile
            tail_position: true,
            //default to false
            loop_tail_position: false,
            //set num_params to size of env
    ...
    fn set_num_params(&self, num: u32) -> Ctxt<'a> {
        Ctxt {
            num_params: num,
            ..self.clone()
        }
    ...
    fn update_tail_pos(&self) -> Ctxt<'a> {
        Ctxt {
            tail_position: false,
            ..self.clone()
        }
    ...
    fn loop_in_tail_position(&self) -> Ctxt<'a> {
        Ctxt {
            loop_tail_position: true,
            tail_position: false,
            ..self.clone()
        }
    }

    fn break_in_loop_tail_pos(&self) -> Ctxt<'a> {
        Ctxt {
            tail_position: true,
            loop_tail_position: false,
            ..self.clone()
...
impl Session {
    //will funs need to be updated to include if it is a tail function?
        ...
        match e {
            ...
            Expr::Let(bindings, body) => {
                ...
                for (var, rhs) in bindings {
                    ...
                    self.compile_expr(&currcx.update_tail_pos(), Loc::Mem(mem), rhs);
                    currcx = nextcx.add_binding(*var, mem);
                }
                //the body expression will be in tail position if the let expresion is in tail position
            ...
            // the content of the unOp will not be in tail position
            Expr::UnOp(op, e) => self.compile_un_op(&cx.update_tail_pos(), dst, *op, e),
            //the content of binOp will not be in tail position
            Expr::BinOp(op, e1, e2) => self.compile_bin_op(&cx.update_tail_pos(), dst, *op, e1, e2),
            Expr::If(e1, e2, e3) => {
                let tag = self.next_tag();
                let else_lbl = format!("if_else_{tag}");
                let end_lbl = format!("if_end_{tag}");

                //condition statement is never in tail position
                self.compile_expr(&cx.update_tail_pos(), Loc::Reg(Rax), e1);
                self.emit_instrs([
                    Instr::Cmp(BinArgs::ToReg(Rax, false.repr32().into())),
                    Instr::Je(else_lbl.clone()),
                ]);
                //the then body will be in tail position if the if expression is in tail position
                self.compile_expr(cx, dst, e2);
                self.emit_instrs([Instr::Jmp(end_lbl.clone()), Instr::Label(else_lbl)]);
                // the else body will be in tail position if theif expression is in tail position
                self.compile_expr(cx, dst, e3);
                self.emit_instr(Instr::Label(end_lbl))
            }
            /* loop and break?
            if loop is in tail position then the corresponding brakes will be in tail position
            will this require something new to be added to the context?
            if break is inside the condition will it still be tail position?
            */
            Expr::Loop(e) => {
                let tag = self.next_tag();
                ...
                if cx.tail_position {
                    self.compile_expr(&cx.set_curr_lbl(&loop_end_lbl).loop_in_tail_position(), dst, e);
                } else {
            ...
            Expr::Break(e) => {
                if let Some(lbl) = cx.curr_lbl {
                    //if the break is in a loop that is in tail position then the breaks body is in tail position
                    if cx.loop_tail_position{
                        self.compile_expr(&cx.break_in_loop_tail_pos(), Loc::Reg(Rax), e);
                    } else {
                        //otherwise it is not in tail position
                        //there should bee no way to reach this if cx.tail_position == true so just as is
            ...
            Expr::Set(var, e) => {
                ...
                //the body of set cannot be ing tail position 
                self.compile_expr(&cx.update_tail_pos(), Loc::Mem(mem), e);
                self.move_to(dst, Arg32::Mem(mem));
            }
            Expr::Block(es) => {
                for e in &es[..es.len() - 1] {
                    //the expresions before the last can never be in tail position
                    self.compile_expr(&cx.update_tail_pos(), Loc::Reg(Rcx), e);
                }
                //the last expr in a block will be in tail position if the block is in tail position
                self.compile_expr(cx, dst, &es[es.len() - 1]);
            }
            /* Call
            if it is in tail position doing tail call
            how to handle changes in arity
            */
            Expr::Call(fun, args) => {
                ...
                for arg in args {
                    let (nextcx, mem) = currcx.next_local();
                    //the arguements for the function will not be in tail position
                    self.compile_expr(&currcx.update_tail_pos(), Loc::Mem(mem), arg);
                    currcx = nextcx;
                }
                // if the expr is in tail position then the function call will also be in tail position
                //for simplicity only dealing with tail call inside function bodies
                if cx.tail_position /*&& cx.in_fun*/ {
                    let mut args_else: Vec<_> = locals(cx.si, args.len() as u32).map(Arg32::Mem).into_iter().collect();
                    if args_else.len() % 2 != 0 {
                        args_else.push(Arg32::Imm(MEM_SET_VAL));
                    }
                    for arg in args_else.iter().rev() {
                        self.emit_instr(Instr::Push(*arg))
                    }
                    //move return pointer rbp and rsp to preven it from being overwritten
                    self.emit_instrs([
                        Instr::Push(Arg32::Mem(mref![Rbp + 16 ])),
                        Instr::Push(Arg32::Mem(mref![Rbp + 8])),
                        Instr::Push(Arg32::Mem(mref![Rbp + 0])),
                    ]);
                    for i in 0..(args_else.len()+3){
                        //seems to be where the issue is
                        let rsp_offset:i32 = (args.len() as i32) * 8 + 2 * 8 - 8 * (i as i32);
                        let rbp_offset:i32 = 16 + 8 * (cx.num_params as i32) - 8 * (i as i32);
                        self.move_to(Loc::Reg(Rax), Arg32::Mem(mref![Rsp + %(rsp_offset)]));
                        self.move_to(Loc::Mem(mref![Rbp + %(rbp_offset)]), Arg32::Reg(Rax))

                    }
                    //add rbp, cx.num_params - args.len() to get it in right spot. only for dif sizesthough
                    /*
                    mov rsp, rbp
                    pop rbp then jump?
                    */
                    // this should make sure that rbp is in the correct spot after everything is copied over
                    let rbp_restore = cx.num_params as i32 - args.len() as i32;
                    self.emit_instr(Instr::Add(BinArgs::ToReg(Rbp, Arg32::Imm(rbp_restore * 8))));
                    self.emit_instr(Instr::Mov(MovArgs::ToReg(Rsp, Arg64::Reg(Rbp))));
                    self.emit_instr(Instr::Pop(Loc::Reg(Rbp)));
                    ...
                    self.call(*fun, locals(cx.si, args.len() as u32).map(Arg32::Mem), args.len() as i32);
            ...
            Expr::MakeVec(size, elem) => {
                ...
                //size is not in tail position
                self.compile_expr(&cx.update_tail_pos(), Loc::Mem(size_mem), size);
                //elem is not in tail position
                self.compile_expr(&nextcx.update_tail_pos(), Loc::Mem(elem_mem), elem);
            ...
            Expr::Vec(elems) => {
                ...
                for elem in elems {
                    let (nextcx, mem) = currcx.next_local();
                    //each elem does is not in tail position
                    self.compile_expr(&currcx.update_tail_pos(), Loc::Mem(mem), elem);
                    currcx = nextcx;
                }
            ...
            Expr::VecSet(vec, idx, elem) => {
                ...
                //vec is not in tail position
                self.compile_expr(&cx.update_tail_pos(), Loc::Mem(vec_mem), vec);
                //idk is not in tail position
                self.compile_expr(&nextcx1.update_tail_pos(), Loc::Mem(idx_mem), idx);
                //elem is not in tail position
                self.compile_expr(&nextcx2.update_tail_pos(), Loc::Reg(Rsi), elem);
            ...
            Expr::VecGet(vec, idx) => {
                ...
                //vec is not in tail position
                self.compile_expr(&cx.update_tail_pos(), Loc::Mem(vec_mem), vec);
                //idx is not in tail position
                self.compile_expr(&nextcx.update_tail_pos(), Loc::Reg(Rdi), idx);
            ...
            Expr::VecLen(vec) => {
                //vec is not in tail position
                self.compile_expr(&cx.update_tail_pos(), Loc::Reg(Rax), vec);
                self.check_is_vec(Rax);
                self.check_is_not_nil(Rax);
                self.emit_instrs([
                    Instr::Sub(BinArgs::ToReg(Rax, Arg32::Imm(1))),
    ...
    fn call(&mut self, fun: Symbol, args: impl IntoIterator<Item = Arg32>, length: i32) {
        let mut args: Vec<_> = args.into_iter().collect();
        if args.len() % 2 == 0 {
            args.push(Arg32::Imm(MEM_SET_VAL));
        }
        //place rsp before moving args
        let rsp_offset = 8 * args.len() + 8;
        self.emit_instr(Instr::Mov(MovArgs::ToMem(mref![Rsp - %(rsp_offset)], Reg32::Reg(Rsp) )));
        for arg in args.iter().rev() {
            self.emit_instr(Instr::Push(*arg))
        }
        //move rsp so that where we put the rsp value to restore to is covered
        self.emit_instr(Instr::Sub(BinArgs::ToReg(Rsp, Arg32::Imm(8))));
        self.emit_instrs([
            Instr::Call(fun_label(fun)),
            //pop rsp to restore it
            Instr::Pop(Loc::Reg(Rsp)),
